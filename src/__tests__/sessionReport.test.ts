...
describe('parseDirtyStat', () => {
  it('parses the full summary line', () => {
    expect(parseDirtyStat('2 files changed, 83 insertions(+), 1 deletion(-), 5 untracked')).toEqual({
      files: 2, insertions: 83, deletions: 1, untracked: 5,
    });
  });

  it('parses singular forms', () => {
    expect(parseDirtyStat('1 file changed, 1 insertion(+), 1 deletion(-)')).toEqual({
      files: 1, insertions: 1, deletions: 1, untracked: 0,
    });
  });

  it('parses untracked-only and empty strings', () => {
    expect(parseDirtyStat('3 untracked')).toEqual({ files: 0, insertions: 0, deletions: 0, untracked: 3 });
...
describe('classifyActivity', () => {
  it('high: >500 lines touched', () => {
    expect(classifyActivity(act({ filesChanged: 1, insertions: 3518, deletions: 10975 }))).toBe('high');
  });
  ...
  it('moderate: mid-size change', () => {
    expect(classifyActivity(act({ filesChanged: 5, insertions: 100, deletions: 50 }))).toBe('moderate');
  });
...
describe('composeSessionReport', () => {
  const sample: RepoActivity[] = [
    ...
    act({ name: 'mid', filesChanged: 5, insertions: 23, deletions: 16, untracked: 1, commitSubjects: ['feat: add tracker'] }),
  ...
  it('renders all sections with correct totals', () => {
    const md = composeSessionReport(sample, 'I worked mostly on big.', new Date('2026-06-13T10:00:00Z'));
  ...
  it('formats entries with stats and uses commit subject as description', () => {
    ...
    expect(md).toContain('- @big: Major uncommitted refactor. (17 files changed, 1509 insertions(+), 1292 deletions(-), 2 untracked)');
    expect(md).toContain('- @mid: feat: add tracker. (5 files changed, 23 insertions(+), 16 deletions(-), 1 untracked)');
